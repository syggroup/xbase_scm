...
IF eTipo_banco="DBF"
   IF Os_IsWin9X()=.F.  // se não for Windows 95 ou 98
      IF UnidadeCorrente()=.F.
         MsgStop("Não é possivel inicar o Sistema apartir dessa Unidade, Favor revisar a instalação do sistema","Aviso do Sistema")
         Return
      ENDIF
   ENDIF
ENDIF
...
IF SESSAO="-DBU" .OR. SESSAO="-dbu" // ABRE UM EDITOR DE REGISTROS PROPRIOS
   IF eCOMSENHA = "S"
      IF LESENHA(eKEY)=.F.
         MsgINFO("Senha de Manutenção do Sistema Incorreta !!!, Favor Verificar","Aviso do Sistema")
         SAIR2()
         Return
      ENDIF
   ENDIF
